...
def my_bilateral(src, msize, sigma, sigma_r, pos_x, pos_y):
    ...
    # for i in range(h):
    #     for j in range(w):
    #         mask = (1/(2 * np.pi * sigma**2)) * np.exp(-((i-x)**2/2*sigma**2)-((j-y)**2/2*sigma**2)) //
    #         * np.exp(-(src(i,j)-src(i+x,i+y))/2*sigma_r**2)
    #


    img_pad = np.zeros((msize, msize))
    ...
    for i in range(h):
        ...
        for j in range(w):
            ...
            dst[i, j] = gaus2D *  np.exp(-(src(i,j)-src(i+x,i+y))/2*sigma_r**2)
